...
def perfil_usuario(
    ...
    # Fase 7: roles personalizados. Si el Admin definió un override de secciones
    # para este usuario, se devuelve para que el frontend lo respete.
    from src.saas.models import PermisosUsuario
    override = db.query(PermisosUsuario).filter(
        PermisosUsuario.usuario_id == usuario_actual.usuario_id,
    ).first()
    secciones = [s for s in override.secciones.split(",") if s] if (override and override.secciones) else None

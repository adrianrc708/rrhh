...
def obtener_usuario_actual(request: Request, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # Fase 7: suspensión por morosidad. Si la empresa del usuario está suspendida,
    # se corta el acceso a toda la plataforma. El SuperAdmin nunca se bloquea (da
    # soporte y gestiona la reactivación).
    if usuario.rol != "SuperAdmin":
        from src.core.models import Empresa
        empresa = db.query(Empresa).filter(Empresa.empresa_id == usuario.empresa_id).first()
        if empresa and empresa.estado == "Suspendida":
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Tu empresa está suspendida por facturación pendiente. Contacta al proveedor del servicio.",
            )

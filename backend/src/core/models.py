...
class HorasPeriodo(Base):
    ...
    fecha_registro = Column(TIMESTAMP, server_default=func.now())
    # Fase 5: validación de sobretiempo por el Gerente. Pendiente | Aprobado | Rechazado.
    # Solo las horas aprobadas se consideran productivas para efectos de pago.
    estado_aprobacion = Column(String(15), nullable=True, default="Pendiente")
    aprobado_por = Column(Integer, ForeignKey("usuarios.usuario_id"), nullable=True)
    fecha_aprobacion = Column(TIMESTAMP, nullable=True)
    is_deleted = Column(Boolean, default=False, nullable=False, index=True)

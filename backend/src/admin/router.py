...
from src.core.models import Empresa, Usuario, ParametroFiscal, Notificacion
...
# ==========================================================================
# Fase 7 — Comunicación masiva (broadcast global del SuperAdmin)
# ==========================================================================

class BroadcastRequest(BaseModel):
    titulo: str
    mensaje: str
    solo_activas: bool = True   # excluir empresas suspendidas/archivadas


@router.post("/broadcast")
def broadcast_global(
    datos: BroadcastRequest,
    db: Session = Depends(get_db),
    usuario_actual: Usuario = Depends(verificar_rol(["SuperAdmin"])),
):
    """Inyecta una notificación en el dashboard de todos los usuarios de todas las empresas."""
    q = db.query(Usuario).filter(Usuario.is_deleted.is_(False), Usuario.rol != "SuperAdmin")
    if datos.solo_activas:
        empresas_activas = [e.empresa_id for e in db.query(Empresa).filter(
            Empresa.estado == "Activa", Empresa.is_deleted.is_(False),
        ).all()]
        q = q.filter(Usuario.empresa_id.in_(empresas_activas))
    usuarios = q.all()
    for u in usuarios:
        db.add(Notificacion(
            empresa_id=u.empresa_id, usuario_id=u.usuario_id,
            titulo=datos.titulo, mensaje=datos.mensaje,
        ))
    db.commit()
    return {"status": "ok", "notificaciones_enviadas": len(usuarios)}

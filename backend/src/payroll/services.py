...
def consolidar_nomina(db: Session, nomina: Nomina, empresa_id: int) -> dict:
    ...
    for empleado in empleados:
        ...
        # Fase 5: el sobretiempo que el Gerente rechazó explícitamente no se paga
        # (horas no productivas). Pendiente/Aprobado y registros previos sí computan.
        horas = db.query(HorasPeriodo).filter(
            ...
            (HorasPeriodo.estado_aprobacion.is_(None)) | (HorasPeriodo.estado_aprobacion != "Rechazado"),
        ).first()
